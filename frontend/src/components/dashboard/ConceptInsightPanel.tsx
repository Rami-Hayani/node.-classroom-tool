...
export default function ConceptInsightPanel({ courseId, concept }: ConceptInsightPanelProps) {
  const [students, setStudents] = useState<ClassStudent[]>([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (!courseId || !concept) { setStudents([]); return; }
    setLoading(true);
    flaskApi.get(`/api/courses/${courseId}/concepts/${concept.id}/students`)
      .then((data) => data as { students: ClassStudent[] })
  ...
  return (
    <div className="rounded-2xl border border-gray-200/80 bg-white p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-widest text-gray-400">Selected concept</p>
